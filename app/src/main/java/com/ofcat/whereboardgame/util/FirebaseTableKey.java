package com.ofcat.whereboardgame.util;

import com.ofcat.whereboardgame.config.AppConfig;

/**
 * Created by orangefaller on 2017/6/21.
 */

public class FirebaseTableKey {

    public static final String TABLE_WAITPLYERROOM = AppConfig.FIREBASE_ROOT_TABLE + "WaitPlayerRoom";
    public static final String TABLE_USER_REPORT = AppConfig.FIREBASE_ROOT_TABLE + "UserReport";
    public static final String TABLE_USER_INFO = AppConfig.FIREBASE_ROOT_TABLE + "UserInfo";
    public static final String TABLE_SYSTEM_CONFIG = AppConfig.FIREBASE_ROOT_TABLE + "SystemConfig";
    public static final String TABLE_SYSTEM_NOTIFICATION = AppConfig.FIREBASE_ROOT_TABLE + "SystemNotification";
    public static final String TABLE_SUGGESTIONS = AppConfig.FIREBASE_ROOT_TABLE + "Suggestions";
    public static final String TABLE_CUSTOM_WAITPLAYERROOM = AppConfig.FIREBASE_ROOT_TABLE + "CustomWaitPlayerRoom";
    public static final String TABLE_STORE_TAB_GENRE = AppConfig.FIREBASE_ROOT_TABLE+ "StoreTabGenre";

    public static final String CUSTOM_STORE_ID = "000999";

}
